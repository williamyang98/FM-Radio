...
void Resample(tcb::span<const Frame<float>> buf_in, tcb::span<Frame<float>> buf_out);

Resampled_PCM_Player::Resampled_PCM_Player(
    std::shared_ptr<RingBuffer<Frame<float>>> _buffer, 
...
void Resampled_PCM_Player::ConsumeBuffer(tcb::span<const Frame<float>> buf) {
