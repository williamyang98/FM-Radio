...
std::shared_ptr<RingBuffer<Frame<float>>> AudioMixer::CreateManagedBuffer(const int nb_blocks) 
{
    auto buf = std::make_shared<RingBuffer<Frame<float>>>(block_size, nb_blocks);
